...
def preprocessed(text_to_process):
    ...
    for symbol in lower_case_text:
        if "a" <= symbol <= "z" or "а" <= symbol <= "я" or "0" <= symbol <= "9" or symbol == " " or symbol == "\n":
            ...
            processed_text += " " + symbol + " "
    return processed_text.strip().replace("  ", " ")


def normalize(text_to_normalize):
    words_to_add = text_to_normalize.split(" ")
    bag_of_words = list(words_to_add)
    for ind, word in enumerate(words_to_add):
        if ind != 0:
            bag_of_words.append(words_to_add[ind - 1] + " " + words_to_add[ind])
    return bag_of_words
...
def train(train_texts, train_labels, pretrain_params=None):
    ...
    label_types = list(sorted(list(set(train_labels))))
    analyzed_texts = {}
    ...
    for label in labeled_word_frequency.keys():
        for word in labeled_word_frequency[label].keys():
...
def classify(texts, params):
    ...
    for bag in bags_of_words:
        ...
        for word in word_occurrences.keys():
            log_of_plausibility += word_occurrences[word] * math.log(
                prob(word, label_types[0]) / prob(word, label_types[1]) + 10**(-7))
